...
def prepare_game():
    ...
    if len(sys.argv) == 2:
        ...
        elif arg in valid_words and len(arg) == NUM_LETTERS:
            secret_word = arg
        else:
...
def is_valid_guess(guess, valid_guesses):
    ...
    return len(guess) == NUM_LETTERS and guess in valid_guesses
...
def get_feedback(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if feedback[i] != CORRECT_COLOR and guessed_word[i] in secret_word_list:
